...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        gameCard.innerHTML = ` 
            ...
            <h3>${games[i].name}</h3>
            <p>Goal: $${games[i].toLocaleString()}</p>
...
const totalContribution = GAMES_JSON.reduce((acc, game) => acc + game.backers, 0);
...
const totalPledged = unfundedGames.reduce((total, game) => total + game.pledged, 0); 
const totalUnfunded = unfundedGames.length;
const fundInfo = `
    A total of $${totalPledged.toLocaleString()} has been pledged for ${totalUnfunded} game${totalUnfunded === 1 ? '' : 's'} that are still unfunded.
`;
console.log(fundInfo);

// create a new DOM element containing the template string and append it to the description container
const paragraph = document.createElement('p');
paragraph.textContent = fundInfo;
...
const [firstGame, secondGame, ...restOfTheGame] = sortedGames;
